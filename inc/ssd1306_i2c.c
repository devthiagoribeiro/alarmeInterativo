...
//Função que desenha um quadrado no display
void ssd1306_draw_square(uint8_t *ssd, uint x, uint y, uint size){
    for(int i = 0; i < size; i++){
        ssd1306_draw_line(ssd, x, y+i, x+size, y+i, true);
    }
}
...
void ssd1306_draw_char_scaled(uint8_t *ssd, int x, int y, uint8_t character, int scale) {
    int idx = ssd1306_get_font(character);
    if (idx == 0) return;
            ...
            if (line & (1 << row)) {
                for (int dx = 0; dx < scale; dx++) {
                    for (int dy = 0; dy < scale; dy++) {
                        ssd1306_set_pixel(ssd, x + col * scale + dx, y + row * scale + dy, true);
                    }
                }
            }
        }
...
void ssd1306_draw_string_scaled(uint8_t *ssd, int x, int y, const char *str, int scale) {
    while (*str) {
        ssd1306_draw_char_scaled(ssd, x, y, *str, scale);
        x += 8 * scale; // Próximo caractere com espaçamento proporcional
        str++;
    }
}


void ssd1306_draw_text_centered(uint8_t *ssd, const char *text, int y, int scale) {
    int text_len = strlen(text);
    int text_width = text_len * 8 * scale;
    int x = (ssd1306_width - text_width) / 2;

    ssd1306_draw_string_scaled(ssd, x, y, text, scale);
}

void ssd1306_draw_text(uint8_t *ssd, int x, int y, const char *text) {
    if (text == NULL) return;
    //ssd1306_draw_string(ssd, x, y, (char *)text);
    ssd1306_draw_text_centered(ssd, (char *)text, y, 2);
}